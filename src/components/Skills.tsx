...
const Skills = () => {
  const categories = [
    ...
    },
    {
      ...
      icon: <Terminal className="text-slate-600 dark:text-slate-300" size={24} />,
        ...
        >
          {categories.map((category, idx) => (
            <motion.div 
              ...
              className="p-8 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 card-hover"
            >
              <div className="mb-6 inline-flex p-3 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
                {category.icon}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, sIdx) => (
                  <span 
                    key={sIdx}
                    className="px-3 py-1 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 text-sm font-medium rounded-lg border border-slate-300 dark:border-slate-700"
                  >
                    {skill}
                  </span>
                ))}
